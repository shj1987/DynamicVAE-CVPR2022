...
class Normal(nn.Module):
    ...
    def _check_inputs(self, size, mu_logvar):
        if size is None and mu_logvar is None:
            raise ValueError(
                'Either one of size or params should be provided.')
        elif size is not None and mu_logvar is not None:
            mu = mu_logvar.select(-1, 0).expand(size)
            logsigma = mu_logvar.select(-1, 1).expand(size).div(2)
            return mu, logsigma
        elif size is not None:
            mu = self.mu.expand(size)
            logsigma = self.logsigma.expand(size)
            return mu, logsigma
        elif mu_logvar is not None:
            mu = mu_logvar.select(-1, 0)
            logsigma = mu_logvar.select(-1, 1).div(2)
            return mu, logsigma
        else:
            raise ValueError(
                'Given invalid inputs: size={}, mu_logvar={})'.format(
                    size, mu_logvar))
