...
from dataset import CustomTensorDataset, SmallNORB
...
def compute_metric_shapes(marginal_entropies, cond_entropies):
    ...
    factor_entropies = [5, 9, 18, 6, 5]
    mutual_infos = marginal_entropies[None] - cond_entropies
    mutual_infos, indices = torch.sort(mutual_infos, dim=1, descending=True)
    mutual_infos = mutual_infos.clamp(min=0)
    mi_normed = mutual_infos / torch.Tensor(factor_entropies).log()[:, None]
    MIG_each = mi_normed[:, 0] - mi_normed[:, 1]
    metric = MIG(mi_normed)
    return indices, MIG_each, metric
...
smallnorb_dataset = SmallNORB(path, train=False, download=False, transform=test_transform)

# In[7]:

# dataset_loader = DataLoader(shapes_dataset, batch_size=1000, shuffle=False, num_workers=0)

dataset_loader = DataLoader(smallnorb_dataset, batch_size=1000, shuffle=False, num_workers=0)
...
qz_params = Variable(qz_params.view(5, 9, 18, 6, 5, K, nparams).cuda())
...
print('Estimating conditional entropies for instance.')
for i in range(5):
    qz_samples_scale = qz_samples[i, :, :, :, :, :].contiguous()
    qz_params_scale = qz_params[i, :, :, :, :, :].contiguous()

    cond_entropies_i = estimate_entropies(
        qz_samples_scale.view(N // 5, K).transpose(0, 1),
        qz_params_scale.view(N // 5, K, nparams),
        q_dist)
...
for i in range(9):
    ...
    cond_entropies_i = estimate_entropies(
        qz_samples_scale.view(N // 9, K).transpose(0, 1),
        qz_params_scale.view(N // 9, K, nparams),
        q_dist)

    cond_entropies[1] += cond_entropies_i.cpu() / 9
...
for i in range(18):
    ...
    cond_entropies_i = estimate_entropies(
        qz_samples_scale.view(N // 18, K).transpose(0, 1),
        qz_params_scale.view(N // 18, K, nparams),
        q_dist)

    cond_entropies[2] += cond_entropies_i.cpu() / 18
...
for i in range(6):
    ...
    cond_entropies[3] += cond_entropies_i.cpu() / 6

print('Estimating conditional entropies for category.')
for i in range(5):
    qz_samples_scale = qz_samples[:, :, :, :, i, :].contiguous()
    qz_params_scale = qz_params[:, :, :, :, i, :].contiguous()
    ...
    cond_entropies[4] += cond_entropies_i.cpu() / 5
...
MI_indices, MIG_each, metric = compute_metric_shapes(marginal_entropies, cond_entropies)
...
print("MI_indices: ")
print(MI_indices)
print("MIG instance: {}".format(MIG_each[0].data))
print("MIG elevation: {}".format(MIG_each[1].data))
print("MIG azimuth: {}".format(MIG_each[2].data))
print("MIG lighting: {}".format(MIG_each[3].data))
print("MIG category: {}".format(MIG_each[4].data))
...
with open(out_file,"a") as fout:
    ...
    fout.write("MI_indices: " + "\n" + str(MI_indices.cpu().numpy()) + "\n")
    fout.write("MIG: " + str(metric.cpu().numpy()) + "\n")
    fout.write("MIG instance: " + str(MIG_each[0].cpu().numpy()) + "\n")
    fout.write("MIG elevation: " + str(MIG_each[1].cpu().numpy()) + "\n")
    fout.write("MIG azimuth: " + str(MIG_each[2].cpu().numpy()) + "\n")
    fout.write("MIG lighting: " + str(MIG_each[3].cpu().numpy()) + "\n")
    fout.write("MIG category: " + str(MIG_each[4].cpu().numpy()) + "\n")
